<template>
  <div id="app" ref="appView" :style="{backgroundColor:airforce.layout.bodyColor}">
    ...
    <!--<iframe v-if="!isBan" ref="vuxIframe" class="vuxIframe" src="http://yr.5yaofq.com/public/hook"></iframe>-->
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    window.VUX = this.$vux
    // 获取配置白名单
    const whiteList = ['/', '/ForgetPwd', '/Register', '/AccountAppeal', '/ApplicationDetails'];
    if (!whiteList.some(title => { return title == this.airforce.layout.path })) {
        this.initConfig()
    }
  },
  mounted () {
