            ...
            <div class="stickyList">
                <z-pull-scroller ref="ZPullScroller" :height="height" @change="change">
                    <panel
                            ...
                            @on-click-item="go"
                            @on-click-footer="go"
                            class="stickyList-panel" :list="stickyList" type="5"></panel>
                </z-pull-scroller>
            </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
      go(item){
          try {
              if(item.recommended == "1" && ( !this.airforce.login_post.data.grade || this.airforce.login_post.data.grade != "0") ){
                    this.$vux.toast.text("vip可见");
                    return;
              }
              if(item.go && item.go.path){
                  this.$router.push(item.go.path);
                  return;
              }
          }catch (e){}
