<template>
  <div class="Xgzl">
      <group class="zg-group" gutter=0>
          ...
          <x-input :value="airforce.prefectInfo.phone" @on-change="airforce.change.set($event,'phone','prefectInfo')"  title="手机号" placeholder="请输入手机号" text-align="right"></x-input>
...
export default {
  ...
  methods: {
    save(){
        ...
        if(this.$utils.is_S(this.airforce.prefectInfo.phone)){
            this.$vux.toast.text("请输入手机号码");
            return;
        }
        if(!this.$utils.isPhone(this.airforce.prefectInfo.phone)){
        ...
        const formData = {
            ...
            phone:this.airforce.prefectInfo.phone,
        }
        this.action({
